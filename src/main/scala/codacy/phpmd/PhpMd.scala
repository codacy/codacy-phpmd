...
object PhpMd extends Tool {
  ...
  override def apply(
    ...
    lazy val nativeConfigParam = {
      configFileNames
        .map(fn => Try(better.files.File(source.path) / fn))
        .collectFirst {
    ...
    val patternConfigParam = {
      configuration.map { patterns =>
        Try(configFromPatterns(patterns))
          .flatMap(fileForConfig)
  ...
  private[this] lazy val defaultRulesPath =
    List("codesize", "cleancode", "controversial", "design", "unusedcode", "naming")
      .map { category =>
  ...
  private[this] def patternIdByRuleNameAndRuleSet(ruleName: String, ruleSet: String)(
      implicit spec: Specification
  ): Option[Pattern.Id] = {
    spec.patterns.collectFirst {
      case pattern
          if pattern.patternId.value == xmlLocation(ruleName, ruleSet) || pattern.patternId.value
            .endsWith(ruleName) =>
        pattern.patternId
    }
  }
  ...
  private[this] def outputParsed(basePath: Path, output: String)(
      implicit spec: Specification
  ): Try[List[_ <: Result]] = {
    Try(XML.loadString(output)).map { outputXml =>
      val issues = (outputXml \ "file").flatMap { file =>
        List(file \@ "name")
          .collect {
            case fname if fname.nonEmpty =>
              relativizeToolOutputPath(basePath, fname)
          }
          .flatMap { fileName =>
            (file \ "violation").flatMap { violation =>
              patternIdByRuleNameAndRuleSet(ruleName = violation \@ "rule", ruleSet = violation \@ "ruleset").flatMap {
                patternId =>
                  Try(
                    Issue(
                      file = fileName,
      ...
      val errors = (outputXml \ "error").map { error =>
        ...
        val message = Option(error \@ "msg").collect {
          case msg if msg.nonEmpty => ErrorMessage(msg)
        }
  ...
  private[this] def defaultParametersFor(
